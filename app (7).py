...
Data model (3 files):
  1. Export file  (MANDATORY) — single sheet from SAP:
       • FA-prefix orders (prefix 1=DM00, prefix 2=LA/EM00) → FG demand + Alt BOM
       • SF01 orders (prefix 5) → component open & confirmed production qty
       • Non-FA LA orders (prefix 2) → component plan order qty
  2. Stock file   (MANDATORY) — 'Stock' sheet only (Req sheet no longer needed)
  3. Receipt file (OPTIONAL)  — GR/receipt quantities to add to stock before MRP

Fixes vs previous version:
  • AttributeError .str on DataFrame → deduplicate cols + safe_series()
  • Date/month column labels preserved exactly as in source file
  • Plan_Order_Qty new column after Open_Production_Qty
"""
...
with st.sidebar:
    ...
    VERIFY_L1 = st.text_input("Verify L1", value="0010748460")
    VERIFY_L2 = st.text_input("Verify L2", value="0010748458")
    VERIFY_L3 = st.text_input("Verify L3 (phantom)", value="0010748814")
    VERIFY_L4 = st.text_input("Verify L4", value="0010300601DEL")
    st.divider()
    st.subheader("Upload files")
    export_file  = st.file_uploader(
        "Export file (.xlsx) — FG demand + production orders ✱",
...
# ═══════════════════════════════════════════════════════════════
# DATE / MONTH PARSING HELPERS  (unchanged from previous version)
# ═══════════════════════════════════════════════════════════════
MONTH_ABBR = {
...
def parse_col_to_date(col, default_year=2026):
    """Parse any date/month column header → (pd.Timestamp, original_label)."""
...
def date_to_label(dt, default_year=2026):
    """Convert a date/Timestamp to a 'Mon-YY' label for grouping."""
    if pd.isna(dt):
        return None
    ts = pd.Timestamp(dt)
...
def safe_series(df, col):
    """Always return a Series even if duplicate column names return a DataFrame."""
    result = df[col]
...
# ═══════════════════════════════════════════════════════════════
# EXPORT FILE PARSER — the key new function
# ═══════════════════════════════════════════════════════════════
def parse_export_file(export_file):
    """
    Parse the SAP export file (single sheet) into three DataFrames:

    1. fg_demand  — FG-level demand for MRP explosion
         columns: BOM_Header, Alt, Month, FG_Demand
         source:  rows where Order prefix is 1 (DM00) or 2 (LA/EM00)
                  AND Production Version starts with 'FA'
         Alt BOM: strip 'FA' from Production Version  (FA10→'10')
         Month:   from Basic finish date, formatted as 'Mon-YY' (Apr-26 etc.)

    2. prod_summary — per-component production order aggregates
         columns: Component, Confirmed_Qty, Open_Production_Qty, Plan_Order_Qty
         sources:
           SF01 (prefix 5) → Open_Production_Qty + Confirmed_Qty
           Non-FA LA (prefix 2) → Plan_Order_Qty  (component planned orders)

    3. months — sorted list of month labels found in fg_demand
    """
    ...
    order_col   = next((c for c in df.columns if c.lower() == "order"), "Order")
    mat_col     = next((c for c in df.columns if "material number" in c.lower()
                        or (c.lower().startswith("material") and "desc" not in c.lower())),
                       "Material Number")
    pv_col      = next((c for c in df.columns if "production version" in c.lower()),
                       "Production Version")
    ord_qty_col = next((c for c in df.columns if "order quantity" in c.lower()), None)
    del_qty_col = next((c for c in df.columns if "quantity delivered" in c.lower()
                        or "delivered" in c.lower()), None)
    conf_col    = next((c for c in df.columns if "confirmed quantity" in c.lower()), None)
    date_col    = next((c for c in df.columns if "finish date" in c.lower()
                        or "basic finish" in c.lower()), None)
    status_col  = next((c for c in df.columns if "system status" in c.lower()
                        or "status" in c.lower()), None)

    # Coerce numeric columns
    for col in [ord_qty_col, del_qty_col, conf_col]:
    ...
    # ── Classify rows by order prefix ────────────────────────
    df["_pfx"] = df[order_col].str[0]
    df["_is_fa"] = df[pv_col].str.upper().str.startswith("FA")

    # FG demand: prefix 1 or 2 AND FA production version
    fg_mask = df["_pfx"].isin(["1","2"]) & df["_is_fa"]
    fg_rows = df[fg_mask].copy()

    # Component production orders: prefix 5 (SF01)
    comp_mask = df["_pfx"] == "5"
    comp_rows = df[comp_mask].copy()

    # Component plan orders: prefix 2 AND non-FA production version
    plan_mask = (df["_pfx"] == "2") & (~df["_is_fa"])
    plan_rows = df[plan_mask].copy()

    # ── Build FG demand ───────────────────────────────────────
    fg_rows["_alt"] = fg_rows[pv_col].str.upper().str.replace("FA","",regex=False).str.strip()
    # Ensure Alt is integer string
    fg_rows["_alt"] = pd.to_numeric(fg_rows["_alt"], errors="coerce").fillna(0).astype(int).astype(str)

    if date_col:
        fg_rows["_month"] = fg_rows[date_col].dt.strftime("%b-%y")
    else:
        fg_rows["_month"] = "Unknown"

    fg_rows = fg_rows[fg_rows["_month"].notna() & (fg_rows["_month"] != "NaT")]

    fg_demand = (
        fg_rows.groupby([mat_col, "_alt", "_month"], as_index=False)[ord_qty_col]
        .sum()
        .rename(columns={mat_col:"BOM_Header", "_alt":"Alt",
                         "_month":"Month", ord_qty_col:"FG_Demand"})
    )
    ...
    all_months = sorted(fg_demand["Month"].unique(), key=month_sort_key)

    # ── Build component production order summary ──────────────
    if ord_qty_col and del_qty_col and conf_col:
        comp_rows["_open"] = (comp_rows[ord_qty_col] - comp_rows[del_qty_col]).clip(lower=0)
        prod_agg = (
            comp_rows.groupby(mat_col, as_index=False)
    ...
    # ── Build component plan order summary ────────────────────
    if ord_qty_col:
        plan_agg = (
    ...
    prod_summary["Confirmed_Qty"]       = prod_summary["Confirmed_Qty"].fillna(0)
    prod_summary["Open_Production_Qty"] = prod_summary["Open_Production_Qty"].fillna(0)
    prod_summary["Plan_Order_Qty"]      = prod_summary["Plan_Order_Qty"].fillna(0)
    prod_summary["Component"]           = prod_summary["Component"].astype(str).str.strip()

    return fg_demand, prod_summary, all_months
...
def build_dot_tree(component, paths, fg_demand_df, months, stock, prod_summary):
    fg_demand_map = {}
    for p in paths:
        rows = fg_demand_df[(fg_demand_df["BOM_Header"]==p["fg"]) & (fg_demand_df["Alt"]==p["alt"])]
        fg_demand_map[(p["fg"],p["alt"])] = float(rows["FG_Demand"].sum()) if not rows.empty else 0
    ...
    for path in paths:
        ...
        prev_id = fg_id
        for comp, desc, qty, sp in zip(path["path_comps"],path["path_descs"],
...
def show_search_section(bom, fg_demand_df, months, stock, prod_summary):
    st.divider()
    st.subheader("🔍 Component Search")
    st.caption("Enter any component code to see demand, shortage, production/plan orders and BOM ancestry tree.")
    ...
    for p in paths:
        rows = fg_demand_df[(fg_demand_df["BOM_Header"]==p["fg"]) & (fg_demand_df["Alt"]==p["alt"])]
        total = float(rows["FG_Demand"].sum()) if not rows.empty else 0
        mv = {m: float(rows[rows["Month"]==m]["FG_Demand"].sum()) for m in months}
        fg_rows.append({"FG code":p["fg"],"Alt":p["alt"],"BOM level":p["level"],
                        "Total demand":f"{total:,.0f}",
                        **{m:f"{mv.get(m,0):,.0f}" for m in months}})
    ...
    dot = build_dot_tree(comp, display_paths, fg_demand_df, months, stock, prod_summary)
    try:
...
def run_mrp(export_file, stock_file, bom_file, receipt_file):
    ...
    if missing_bom:
        st.error(f"Missing required BOM columns: {missing_bom}"); return None
    ...
    # ── SECTION 2: EXPORT FILE → FG demand + production orders
    with status:
        st.write("► Parsing Export file (FG demand + production orders) ...")

    try:
        fg_demand, prod_summary, months = parse_export_file(export_file)
    except Exception as e:
        st.error(f"Export file parse error: {e}")
        return None

    if fg_demand.empty:
        st.error("No FG demand rows found in export file. "
                 "Ensure file has FA-prefix production versions and order prefixes 1 or 2.")
        return None
    ...
    log(f"Months: {months}")
    log(f"Component production orders: {(prod_summary['Open_Production_Qty']>0).sum()} components")
    log(f"Component plan orders: {(prod_summary['Plan_Order_Qty']>0).sum()} components")

    # Build req_long in the same format the MRP engine expects
    req_long = fg_demand.rename(columns={"BOM_Header":"BOM Header", "FG_Demand":"FG_Demand"})
    # req_long columns: BOM Header, Alt, Month, FG_Demand
    ...
    try:
        # Try reading "Stock" sheet; fall back to first sheet
        try:
        ...
        st.error(f"Stock file error: {e}"); return None
    ...
    if not receipt_qty.empty:
        st.info(f"ℹ️ Receipt quantities applied for {receipt_added} components before MRP.")
    ...
    def show_verify(tab, result_df, target, level_label):
        with tab:
            ...
            st.caption(f"Description: {t['Description'].iloc[0]} | "
                       f"Stock (post-receipt): {stock.get(target,0):,.3f}")
    ...
    # Column order: Component | Desc | Proc type | Sp.Proc | Confirmed | Open PO | Plan PO | Stock | [Receipt] | months
    ...
    for c in final_cols:
        if c not in pivot.columns:
            num_cols = month_cols + ["Confirmed_Qty","Open_Production_Qty","Plan_Order_Qty","Stock","Receipt_Qty"]
            pivot[c] = 0 if c in num_cols else ""
    ...
    st.caption(f"{len(pivot):,} rows · {len(month_cols)} month columns · cumulative shortage")
    ...
    return dict(bom=bom, fg_demand=fg_demand, months=months, stock=stock,
